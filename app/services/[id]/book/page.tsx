...
export default async function BookServicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!service) notFound();
  return <section className="bg-ivory py-10"><div className="container-shell max-w-3xl"><DashboardHeader profile={profile} title="Book this mentor service" description="Share your goal clearly, choose your preferred slot, and continue through the secure test checkout." /><ServiceBookingForm serviceId={service.id} title={service.title} price={Number(service.price)} /></div></section>;
}
