  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-6 p-6 sm:p-8">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Service title"><input required className="form-input" value={formData.title} onChange={(event) => update("title", event.target.value)} placeholder="Example: Mock Interview for Product Roles" /></Field>
      ...
      <Field label="Description"><textarea required rows={4} className="form-input" value={formData.description} onChange={(event) => update("description", event.target.value)} placeholder="Explain the problem this service solves and how the session works." /></Field>
      <div className="grid gap-5 lg:grid-cols-2">
        <Field label="Who is this for?"><textarea required rows={4} className="form-input" value={formData.target_audience} onChange={(event) => update("target_audience", event.target.value)} placeholder="Fresh graduates preparing for software interviews..." /></Field>
        <Field label="What will the user get?"><textarea required rows={4} className="form-input" value={formData.deliverables} onChange={(event) => update("deliverables", event.target.value)} placeholder="Live simulation, structured feedback, and action plan..." /></Field>
      </div>
